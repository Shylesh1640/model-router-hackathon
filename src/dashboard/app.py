...
def broadcast(route_response: RouteResponse):
...
async def get_history(limit: int = Query(50, ge=1, le=200)):
    ...
    return [
        _response_to_dict(r) for r in pipeline.get_history(limit=limit)
    ]
...
def _response_to_dict(r: RouteResponse) -> dict:
    return {
        "query": r.query,
...
<style>
  ...
  .tier-deep { border-left: 4px solid #ef4444; border-left: 4px solid #a855f7; }
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-5 gap-4 mb-8" id="stats-row">
      ...
      <div class="card text-center">
        ...
        <div class="text-xs text-slate-400">Deep (paid)</div>
      </div>
      <div class="card text-center">
...
async function testRoute() {
  ...
  try {
    ...
    // WebSocket will push it, but also add it directly
    addRouteEntry({
      type: 'route',
      query: data.query,
